...
// sr_nn_top_img
// -----------------------------------------------------------------------------
// Phase9.5 image-level CNN hierarchy top.
//
// This is the 128x128 LR image version of the reference-like hierarchy:
//   parameter ROM IP + Conv1 + Conv1-to-Conv3 slice + Conv3
//   + image-size RAM IP + PixelShuffle + output pack.
// -----------------------------------------------------------------------------

module sr_nn_top_img (
    input wire clk,
    input wire rst,
    ...
    input wire [13:0] dbg_c3_rd_addr,
    output wire [31:0] dbg_c3_rd_data,

    input wire dbg_out_rd_en,
    input wire [14:0] dbg_out_rd_addr,
    ...
    conv3_feature_ram_img u_conv3_feature_ram_img (
        .clka(clk),
    ...
    output_image_ram_img u_output_image_ram_img (
        .clka(clk),
        .ena(output_ram_wr_en_a | output_ram_dbg_en),
        .wea(output_ram_wr_en_a),
        .addra(output_ram_dbg_en ? output_ram_dbg_addr : output_ram_wr_addr_a),
